...
var province_obj = ["北京市","天津市","河北省","山西省","内蒙古","辽宁省","吉林省","黑龙江省","上海市","江苏省","浙江省","安徽省","福建省","江西省","山东省","河南省","湖北省","湖南省","广东省","广西自治区","海南省","重庆市","四川省","贵州省","云南省","西藏自治区","陕西省","甘肃省","青海省","宁夏回族自治区","新疆维吾尔自治区","香港特别行政区","澳门特别行政区","台湾省","其它"];
var city_obj = [["北京","东城区","西城区","崇文区","宣武区","朝阳区","丰台区","石景山区"," 海淀区（中关村）","门头沟区","房山区","通州区","顺义区","昌平区","大兴区","怀柔区","平谷区","密云县","延庆县","其他"], 
		...
		["澳门","其他"],
		["台湾","其他"]];
function func_select_province_html()
{
	var h = '<div class="form_top"></div><ul class="menu_list01">';
	var len = province_obj.length;
	var li_class = '';
	var select_class = '';
	for(var i = 0 ; i < len ; i += 1)
	{
		li_class = 'province_item menu_item02';
		select_class = 'm01_right_select';
		switch (i)
		{
			case 0 :
				li_class += ' menu_item01_t';
				break;
			case len - 1 :
				li_class += ' menu_item01_b';
				break;
		}
		if( i == current_province_id )
		{
			select_class += ' m01_right_selected';
		}
		h += '<li data-id="' + i + '" class="' + li_class + '"><span>' + province_obj[i] +
			'</span><span class="' + select_class + '"></span></li>';
	}
	h += '</ul><div class="form_bottom"></div>';
	return h;
}
function func_select_city_html()
{
	var h = '<div class="form_top"></div><ul class="menu_list01">';
	var city_item = city_obj[ current_province_id ];
	var len = city_item.length;
	var li_class = '';
	var select_class = '';
	for(var i = 0 ; i < len ; i += 1)
	{
		li_class = 'city_item menu_item02';
		select_class = 'm01_right_select';
		switch (i)
		{
			case 0 :
		...
		if( i == current_city_id )
		{
			select_class += ' m01_right_selected';
		}
		h += '<li data-id="' + i + '" class="' + li_class + '"><span>' + city_item[i] 
			+ '</span><span class="' + select_class + '"></span></li>';
	}
	h += '</ul><div class="form_bottom"></div>';
	return h;
...
{
	$('#page3').html(func_pagecom_html(3, '选择省份'));
	scrollLeft();
	touch_btn ('back_icon3');
	$('#back_icon3').singleTap(scrollRight);
	myScroll3 = new iScroll('wrapper3', { checkDOMChanges: false});
	$('#scroller3').html( func_select_province_html() );
	$('.province_item').each(function(index)
	{
        touch_bg(this, 'bg_gra01');
        $(this).singleTap(function()
        {
        	scrollRight();
        	if ($(this).attr('data-id') != current_province_id)
        	{
        		current_city_id = 0;
        	}
	        current_province_id = $(this).attr('data-id');
	        var province_text = province_obj[ current_province_id ];
	        var city_text = city_obj[ current_province_id ][0];
	        $('#province .m01_right_in').val( province_text );
	        $('#city .m01_right_in').val( city_text );
		});
	});
	setTimeout(function ()
	{
...
{
	$('#page3').html(func_pagecom_html(3, '选择城市'));
	scrollLeft();
	touch_btn ('back_icon3');
	$('#back_icon3').singleTap(scrollRight);
	myScroll3 = new iScroll('wrapper3', { checkDOMChanges: false});
	$('#scroller3').html( func_select_city_html() );
	$('.city_item').each(function(index)
	{
        touch_bg(this, 'bg_gra01');
        $(this).singleTap(function()
        {
        	scrollRight();
	        current_city_id = $(this).attr('data-id');
	        var city_text = city_obj[ current_province_id ][ current_city_id ];
	        $('#city .m01_right_in').val( city_text );
		});
	});
	setTimeout(function ()
