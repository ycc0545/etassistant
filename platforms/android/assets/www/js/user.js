...
function login()
{
	...
	my_toast ('登录中', 0);
	var url = 'http://itms.yinhebus.com/api/app/Tourguide/CheckSignIn.ashx?';
	pwd = hex_md5(pwd);
	url += 'G_UserName='+ username+ '&G_PWord='+ pwd;
	var timeParam = Math.round(new Date().getTime()/1000);
	url += "&timenow="+ timeParam;
	$.getJSON(url, function(data)
	{
		if(data['SignInStatus'] == 'pass')
		{
			my_toast('登录成功', 2500);
			localStorage.setItem('GuideCode', data['GuideCode']);
			into_main();
			return;
		}
		my_toast('用户名或密码错误', 2500);
	})
}
